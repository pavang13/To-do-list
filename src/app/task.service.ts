...
export class TaskService {
  ...
  highprioritytask(emailId:any){
    this.token=localStorage.getItem('tgt')||''
    console.log(this.token)
    this.reqHeader=new HttpHeaders().set("Authorization",'Bearer '+this.token).set('access-control-allow-origin', '*');
    return this.httpClient.get<any>(this.baseurl+"/gettask/"+emailId+"/High",{headers:this.reqHeader});


  }

  mediumprioritytask(emailId:any){
    this.token=localStorage.getItem('tgt')||''
    console.log(this.token)
    this.reqHeader=new HttpHeaders().set("Authorization",'Bearer '+this.token).set('access-control-allow-origin', '*');
    return this.httpClient.get<any>(this.baseurl+"/gettask/"+emailId+"/Medium",{headers:this.reqHeader});


  }

  lowprioritytask(emailId:any){
    this.token=localStorage.getItem('tgt')||''
    console.log(this.token)
    this.reqHeader=new HttpHeaders().set("Authorization",'Bearer '+this.token).set('access-control-allow-origin', '*');
    return this.httpClient.get<any>(this.baseurl+"/gettask/"+emailId+"/Low",{headers:this.reqHeader});
